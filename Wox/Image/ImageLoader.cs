namespace Wox.Image
{
    ...
    public static class ImageLoader
    {
        ...
        {
            Logger.WoxDebug($"load begin {path}");
            var img = _cache.GetOrAdd(path, _defaultFileImage, p =>
                {
                    try
                    {
                        return LoadInternal(p);
        ...
        private static ImageSource LoadInternal(string path)
        {
            ...
            if (path.StartsWith(key)) return Image.EmbeddedIcon.GetImage(key, path, 32);
            ...
            if (!Path.IsPathRooted(path) && normalImage) path = Path.Combine(Constant.ProgramDirectory, "Images", Path.GetFileName(path));
